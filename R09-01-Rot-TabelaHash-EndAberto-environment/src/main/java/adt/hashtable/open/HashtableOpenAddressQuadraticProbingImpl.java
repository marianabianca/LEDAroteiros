...
public class HashtableOpenAddressQuadraticProbingImpl<T extends Storable> extends AbstractHashtableOpenAddress<T> {
   ...
   public int indexOf(T element) {
      ...
      if (element != null) {
         ...
         while (this.getElement(returnIndex) != null && !this.getElement(returnIndex).equals(element)
               && probe != super.table.length) {
            probe++;
            returnIndex = this.getHash(element, probe);
         }

         if ((this.getElement(returnIndex) == null)
               || (this.getElement(returnIndex) != null && !this.getElement(returnIndex).equals(element))) {
            returnIndex = -1;
         }
      }
