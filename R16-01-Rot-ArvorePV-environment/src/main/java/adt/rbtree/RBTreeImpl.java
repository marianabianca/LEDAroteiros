...
public class RBTreeImpl<T extends Comparable<T>> extends BSTImpl<T>
	...
	private boolean verifyChildrenOfRedNodes(RBNode<T> node) {
		...
		if (!node.isEmpty()) {
			if (this.isBlack(node)) {
			returnResult = this.verifyChildrenOfRedNodes((RBNode<T>) node.getLeft()) && this.verifyChildrenOfRedNodes((RBNode<T>) node.getRight());
	...
	public RBNode<T>[] rbPreOrder() {
		...
		RBNode<T>[] returnArray = new RBNode[this.size()];
 		this.preOrder((RBNode<T>) this.getRoot(), returnArray);
 		
 		return returnArray;
	}
	
	private void preOrder(RBNode<T> node, RBNode<T>[] returnArray) {
 		if (!node.isEmpty()) {
 			this.addElementToArray(returnArray, node);
 			this.preOrder((RBNode<T>) node.getLeft(), returnArray);
 			this.preOrder((RBNode<T>) node.getRight(), returnArray);
 		}
 	}
	
 	public void addElementToArray(RBNode<T>[] returnArray, RBNode<T> node) {
 		int i = 0;
