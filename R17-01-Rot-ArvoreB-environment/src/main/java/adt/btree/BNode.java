...
public class BNode<T extends Comparable<T>> {
	...
	public BNode<T> getMinors() {
		BNode<T> node = new BNode<>(this.order);
		
		int aux = (order/2);
		
		for (int i = 0; i < aux; i++) {
			node.addElement(this.getElementAt(i));
			if (!this.isLeaf()) {
				node.addChild(i, this.getChildren().get(i));
			}
		}
		
		if (!this.isLeaf()) {
			node.addChild(aux, this.getChildren().get(aux));
		}
		
		return node;
	}
	
	public BNode<T> getLargers() {
		BNode<T> node = new BNode<>(this.order);
		...
		for (int i = aux; i < order; i++) {
			node.addElement(this.getElementAt(i));
			if (!this.isLeaf()) {
				node.addChild(i - aux, this.getChildren().get(i));
			}
		}
		
		if (!this.isLeaf()) {
			node.addChild((this.order) - aux, this.getChildren().get(this.order));
		}
